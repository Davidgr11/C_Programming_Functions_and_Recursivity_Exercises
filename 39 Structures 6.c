...
int  buscar_cuentas(P[10],int,N,C [3]);
N buscar_cliente(P[10],int,int);

int main()
{
    P vectorP[10],personaE;
    int opc,dir=0,exito,i,cuentaB;
    N nombreB;
    D direccionE;
    char rfcE[11];
    C ctasE[3];
    ...
    do
    {
        printf("\n 1. Llenar \n 2. Imprimir \n 3. Buscar por nombre \n 4. Buscar direccion \n 5. Buscar RFC de una persona \n 6. Buscar cuentas de cliente\n 7. Buscar cliente de una cuenta\n 0. Salir");
        scanf("%d",&opc);
        switch(opc)
        {
            ...
            case 6:printf("\n Ingresa el nombre completo de persona a buscar ");
                ...
                fflush(stdin);
                gets(nombreB.apm);
                exito=buscar_cuentas(vectorP,dir,nombreB,ctasE);
                if (exito ==1)
                   for (i=0;i<3;i++)
                       printf("\n Cuenta Num %d Saldo %f  %s\n",ctasE[i].numcta,ctasE[i].saldo,ctasE[i].tipo);
                else
                    printf("\n No existe esa persona ");
                break;
            case 7: printf("\n Ingresa el numero de cuenta a buscar");
                    scanf("%d",&cuentaB);
                    nombreB=buscar_cliente(vectorP,dir,cuentaB);
                    if (strcmp(nombreB.np,"\0")!=0)
                        printf("\n La cuenta pertenece a %s %s %s\n",nombreB.np,nombreB.app,nombreB.apm);
                    else
...
N buscar_cliente(P vecP[10],int dir,int cuentaB)
{
   int i=0,j,band=0;
   N estvacia={NULL};
   while(i<dir && band==0)
   {
      ...
      while (j<3 && band==0)
      {
          if (cuentaB==vecP[i].vctas[j].numcta)
            band=1;
          else
            j++;
      }
   ...
   if(band==1)
    return (vecP[i].nombre);
   else
    return estvacia;

}

int buscar_cuentas(P vecP[10],int dir ,N nomB,C ctas [3])
{
    int i=0,j;
